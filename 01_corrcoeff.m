...
%  - λ value for which correlation coefficient ρ is strongest between E and ABC_λ
%  - λ value for which ρ is strongest between E and H_λ
%  - λ value for which ρ is strongest between ΔH and ABC_λ
%  - λ value for which ρ is strongest between ΔH and H_λ
...
numData = size(expData, 2);       % two (E & ΔH)
...
xfar = linspace(-20,20,800); % xfar is same for both E and ΔH
...
for ii = 1:numData
  ...
  % Indicate inverval for which ABC_λ is the better indicator (before corr-curves)

  % WARNING: these xmeet1-ymeet2 values are hardcoded, computed separately
  %          ... These are coordinates where ρ-λ of {ΔH or E}-ABC_λ
  %          ... intersects ρ-λ of {ΔH or E}-H_λ
  xmeet1 = [
    -0.46299; % for E
    -2.2887   % for ΔH
  ...
  for n = 1:numIndices
    % Function to get corrcoef ρ between E/ΔH (depending on ii) with specified λ
    %                                and ABC_λ/H_λ (depending on n)
    get_indices_vals = @(lambda) getIndexFns{n}(lambda)(!isnan(expData{1,ii}));
    ccFn = @(lambda) corrcoef(
      get_indices_vals(lambda), % Either ABC(lambda) or H(lambda)
      expData{1,ii}(!isnan(expData{1,ii})) % E/ΔH
    )(1,2);
    ...
    % Generate curve label                  [E,ΔH]         [ABC,H]
  ...
  % Mark and write on the plot the limits of lambda where ABC_λ is better than H_λ
